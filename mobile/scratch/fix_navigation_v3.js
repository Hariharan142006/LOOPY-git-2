      ...
      dynamicMappings.forEach(m => {
        if (m.pattern.test(content)) {
          content = content.replace(m.pattern, (match, idVar) => {
            return `${m.screen}', { id: ${idVar} })`; // Added closing parenthesis
          });
          // Fix the leading quote/backtick that might be left over
          content = content.replace(new RegExp("navigate\\(['\"\`]" + m.screen, 'g'), `navigate('${m.screen}`);
          content = content.replace(new RegExp("replace\\(['\"\`]" + m.screen, 'g'), `replace('${m.screen}`);
          
          // Cleanup artifacts from multiple replacements
          content = content.replace(new RegExp(`${m.screen}', { id: ([^}]+) }\\)\\)`, 'g'), `${m.screen}', { id: $1 })`); // Fix double ))
          changed = true;
        }
      });
