...
// Make sure to create this CSS file or import it appropriately
...
class App {
    ...
    constructor(
        ...
        try {
            ...
            console.warn('WebGL initialization failed, gallery scanning disabled', e);
            this.destroy();
            this.renderer = null;
        }
    }
    createRenderer() {
        ...
        try {
            ...
            throw new Error("Failed to create WebGL Renderer: " + (e as Error).message);
        }
    }
    createCamera() {
        if (!this.gl) return;
...
import { isWebGLSupported } from '../../utils/webgl-check';
...
export default function CircularGallery({
    ...
    }, [isSupported, items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase]);
    ...
    if (isSupported === false) {
        ...
        return (
            <div className="flex flex-wrap justify-center gap-4 p-4">
                {(items || []).map((item, i) => (
                    <div key={i} className="relative rounded-lg overflow-hidden group w-[300px] h-[200px]">
                        <img src={item.image} alt={item.text} className="w-full h-full object-cover" />
                        <div className="absolute inset-0 flex items-center justify-center bg-black/40 text-white opacity-100 font-bold text-xl">
                            {item.text}
                        </div>
                    </div>
                ))}
